...
=M-x describe-face= to inquire the current font and set it here.
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (cond ((eq system-type 'windows-nt)
        ...
        ((eq system-type 'darwin)
         ...
         (set-face-attribute 'default nil :family "Source Code Pro" :foundry "ADBO" :height 160))
        ;; (t (set-face-attribute 'default nil :font "Courier 10 Pitch" :height 136))
        ;; looks like gnu/linux is not working in system-type
        ;; (t (set-face-attribute 'default nil :family "Source Code Pro" :foundry "ADBO" :height 143))
        (t (set-face-attribute 'default nil :family "JetBrainsMono Nerd Font" :foundry "JB" :height 137))
        )

  ;; (set-face-attribute 'fixed-pitch nil :family "Source Code Pro" :foundry "ADBO" :height 136)
  (set-face-attribute 'default nil :family "JetBrainsMono Nerd Font" :foundry "JB" :height 137)
  (set-face-attribute 'fixed-pitch nil :family "JetBrainsMono Nerd Font" :foundry "JB" :height 136)
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  ;; above won't work on 29.3 emacs, tabs still tabs
  ;; below is "linux" with tabs to spaces
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  ;;                (setq org-agenda-path "~/Documents/org/")) ; this is Windows map of /usr2/xni
                  (setq org-agenda-path "y:/xni/org/")) ; company machine share win mount of DevCompute
               (t
  ;;                (setq org-agenda-path "~/Documents/org/")))
                  (setq org-agenda-path "/local/mnt/workspace/xni/org/"))) ;company DecCompute linux
              ...
              ;; (setq org-agenda-files '("~/Documents/org"))
                (defun my/org-refresh-agenda-files (&rest _)
                  (setq org-agenda-files
                        (and (file-directory-p org-agenda-path)
                             (directory-files-recursively org-agenda-path "\\.org$"))))
                (setq org-agenda-files
                      (delq nil
                            (mapcar (lambda (file)
                                      (and (file-exists-p file) file))
                                    (list org-todo-file org-log-file))))
                (advice-add 'org-agenda :before #'my/org-refresh-agenda-files)
                (advice-add 'org-refile :before #'my/org-refresh-agenda-files)
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package org-roam
    ...
    :custom
    (org-roam-directory (concat org-agenda-path "org-roam"))
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (use-package consult-projectile
    ...
    :commands (consult-projectile projectile-switch-project))
...
** exec path
   ...
   <2024-06-05 Wed> looks like I have to enable this for linux as well, because when launch it NOT from terminal, it won't get the =.bashrc= sourced.
   #+BEGIN_SRC emacs-lisp :tangle yes
     (cond ((eq system-type 'darwin)
