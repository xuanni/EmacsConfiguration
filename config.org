...
** helm
   #+BEGIN_SRC emacs-lisp :tangle yes
     ...
     ;; replacing isearch-forward with helm-occur will make keyboard macro hard
     ;; to work with, now bind another key to isearch-forward, note that M-s w
     ;; is isearch-forward-word
     (global-set-key (kbd "M-s f") 'isearch-forward)
