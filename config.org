...
*Problems*
- ivy action (M-o) in switch projects won't show all options in the minibuffer, and also there is no option to go to dired in the action list.
...
* counsel-projectile
#+BEGIN_SRC emacs-lisp :tangle yes
  ;; add an action to open dired at root of project
